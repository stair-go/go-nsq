...
// ConnectAllNSQDByLookupdAddr Connect All nsqd addresses
//
// Topic and channel will be created automatically
// Avoid creating and polling lookupd manually
// Pairing with LProducer will allow you to use NSQ better, Of course, you can also use it alone
//
// Suitable for small and medium-sized nsqd clusters
func (r *Consumer) ConnectAllNSQDByLookupdAddr(lookupdAddr string) error {
	if atomic.LoadInt32(&r.stopFlag) == 1 {
		return errors.New("consumer stopped")
	}
	if atomic.LoadInt32(&r.runningHandlers) == 0 {
	...
	if err := validatedLookupAddr(lookupdAddr); err != nil {
		return err
	}
	...
	r.lookupdHTTPAddrs = append(r.lookupdHTTPAddrs, lookupdAddr)
	numLookupd := len(r.lookupdHTTPAddrs)
	r.mtx.Unlock()
...
// ConnectToAllNSQDByNsqLookupds Connect All nsqd addresses
//
// Topic and channel will be created automatically
// Avoid creating and polling lookupd manually
// Pairing with LProducer will allow you to use NSQ better, Of course, you can also use it alone
//
// Suitable for small and medium-sized nsqd clusters
func (r *Consumer) ConnectToAllNSQDByNsqLookupds(nsqLookupds []string) error {
	for _, addr := range nsqLookupds {
		err := r.ConnectAllNSQDByLookupdAddr(addr)
		if err != nil {
			return err
		}
	}
...
// poll all known lookup servers every LookupdPollInterval
func (r *Consumer) lookupdAllLoop() {
	// add some jitter so that multiple consumers discovering the same topic,
	// when restarted at the same time, dont all connect at once.
	r.rngMtx.Lock()
	jitter := time.Duration(int64(r.rng.Float64() *
...
exit:
	...
	r.log(LogLevelInfo, "exiting lookupdAllLoop")
	r.wg.Done()
}
...
// return the next lookupd endpoint to query
// keeping track of which one was last used
func (r *Consumer) nextLookupdEndpointAll() string {
	r.mtx.RLock()
	if r.lookupdQueryIndex >= len(r.lookupdHTTPAddrs) {
		r.lookupdQueryIndex = 0
	}
...
func (r *Consumer) queryLookupdAll() {
	retries := 0

retry:
	endpoint := r.nextLookupdEndpointAll()
	r.log(LogLevelInfo, "querying nsqlookupd %s", endpoint)
